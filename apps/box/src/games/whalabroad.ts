...
export interface WhalabroadState {
  ...
  // Bots: synthetic players that have a guestId in passOrder/scores but NO
  // socket. Their actions are auto-generated by botDecide* in resolveTurn.
  // Display name is "Bot Captain Foo" etc. Counted toward minimum-player check.
  bots: string[]; // guestIds that are bots
}
...
export function startGame(
  ...
  if (members.length < 1) {
    throw new Error("whalabroad: need at least 1 player to host");
  }
  // Note: full 3-player minimum (1 whale + 2 whalers) is enforced at
  // commitLobby() time. Solo hosts can fill seats with bots before
  // committing.
...
// Pool of bot captain names — picked round-robin so multiple bots in one
// game don't all share a name.
const BOT_NAMES = [
  "Bot Ahab",      "Bot Starbuck", "Bot Stubb",     "Bot Flask",
  "Bot Queequeg",  "Bot Pip",      "Bot Daggoo",    "Bot Tashtego",
];

export function addBot(roomId: string, hostGuestId: string): WhalabroadState | null {
  const s = sessions.get(roomId);
  if (!s || s.phase !== "lobby") return null;
  // Soft cap at 6 bots (game caps at 7 ships; +1 whale = 8 max players).
  if (s.bots.length >= 6) return null;
  // Generate unique guestId. The "bot_" prefix is the canonical marker the
  // server uses to detect bots without consulting state.bots — useful in
  // contexts where state may not be available.
  const botId = `bot_${nanoid(8)}`;
  const taken = new Set(s.scores.map(x => x.displayName));
  let name = BOT_NAMES[s.bots.length % BOT_NAMES.length];
  // De-duplicate name if collision (rare — only if a human used the same)
  let suffix = 2;
  while (taken.has(name)) { name = `${BOT_NAMES[s.bots.length % BOT_NAMES.length]} ${suffix++}`; }
  s.bots.push(botId);
  s.passOrder.push(botId);
  s.scores.push({
    guestId: botId, displayName: name, score: 0,
    role: "whaler", outcome: "draw",
  });
  // Bots aren't volunteers for whale by default — they take whatever role
  // is assigned at commit time. Host can edit if we want bot-volunteers later.
  return s;
}

export function removeBot(roomId: string, hostGuestId: string, botId: string): WhalabroadState | null {
  const s = sessions.get(roomId);
  if (!s || s.phase !== "lobby") return null;
  if (!s.bots.includes(botId)) return null;
  s.bots = s.bots.filter(id => id !== botId);
  s.passOrder = s.passOrder.filter(id => id !== botId);
  s.scores = s.scores.filter(x => x.guestId !== botId);
  s.whaleVolunteers = s.whaleVolunteers.filter(id => id !== botId);
  return s;
}

export function isBot(s: WhalabroadState, guestId: string): boolean {
  return s.bots.includes(guestId);
}
...
export function resolveTurn(roomId: string): WhalabroadState | null {
  ...
  // 0. Auto-fill bot actions for any bot that hasn't had its action queued
  //    yet (which is always — bots have no socket). Decision functions are
  //    pure reads on `s` so it's fine to run them just before resolution.
  for (const botId of s.bots) {
    if (s.whale && s.whale.guestId === botId) {
      if (!s.whaleAction) {
        const a = botActionFor(s, botId) as WhalePending | null;
        if (a) s.whaleAction = sanitizeWhaleAction(a);
      }
    } else {
      if (!s.shipActions[botId]) {
        const a = botActionFor(s, botId) as ShipPending | null;
        if (a) s.shipActions[botId] = sanitizeShipAction(a);
      }
    }
  }
...
// BOT AI
// ──────────────────────────────────────────────────────────────────────────
// Each bot's decision is a pure function of the current state + the bot's
// guestId. Called once per turn from auto-fill in resolveTurn for any bot
// that hasn't had an action queued by humans (which never happens for bots,
// since they have no socket — but the same code path is used for forgetful
// humans too).

// Helpers ----------------------------------------------------------------

function aliveShips(s: WhalabroadState): ShipState[] {
  return s.ships.filter(sh => !sh.sunk);
}

// Find the closest live ship to (x, y) by chebyshev distance.
function nearestShip(s: WhalabroadState, x: number, y: number): ShipState | null {
  let best: ShipState | null = null;
  let bestD = Infinity;
  for (const sh of aliveShips(s)) {
    const d = chebyshev(x, y, sh.x, sh.y);
    if (d < bestD) { best = sh; bestD = d; }
  }
  return best;
}

// Pick the cardinal step (dx, dy ∈ {-1,0,1}) that moves closest from
// (fx, fy) toward (tx, ty). Returns null if already there.
function stepToward(fx: number, fy: number, tx: number, ty: number): { dx: number; dy: number } | null {
  if (fx === tx && fy === ty) return null;
  return { dx: Math.sign(tx - fx), dy: Math.sign(ty - fy) };
}

// Pick a random water cell on the board for "wander" fallback. Used when
// no clear target — keeps games progressing.
function randomWaterCell(s: WhalabroadState): { x: number; y: number } {
  const candidates: Array<[number, number]> = [];
  for (let y = 0; y < BOARD_SIZE; y++) for (let x = 0; x < BOARD_SIZE; x++) {
    if (cellTypeAt(s, x, y) === "water") candidates.push([x, y]);
  }
  const [x, y] = candidates[Math.floor(Math.random() * candidates.length)] ?? [6, 6];
  return { x, y };
}

// Whale decision -----------------------------------------------------------

function botDecideWhale(s: WhalabroadState): WhalePending {
  if (!s.whale || s.whale.dead) return { kind: "pass" };
  const w = s.whale;

  // Count surface vs submerged in last 4 turns of history (window minus the
  // proposed turn = 4 entries). If we've been submerged 3 of last 4, we MUST
  // surface this turn or the rolling rule rejects future submerges too.
  const recent = w.history.slice(-(config.rollingWindow - 1));
  const submergedCount = recent.filter(t => t === "submerged" || t === "bubble").length;
  const surfaceCount = recent.filter(t => t === "surface").length;
  const mustSurface = submergedCount >= config.maxSubmergedInWindow ||
                      (recent.length === config.rollingWindow - 1 && surfaceCount < config.minSurfaceInWindow - 1);

  const target = nearestShip(s, w.x, w.y);

  if (mustSurface || w.surfaced) {
    // Already on surface, or rolling rule forces it. Try to ram if a ship
    // is adjacent. Else surprise-breach if one is adjacent and we were
    // submerged. Else just breach toward target.
    if (target && chebyshev(w.x, w.y, target.x, target.y) <= 1) {
      // We CAN ram. Surface ram is 2 dmg.
      return { kind: "ram_strike", dx: Math.sign(target.x - w.x), dy: Math.sign(target.y - w.y), steps: 1, targetGuestId: target.guestId };
    }
    if (!w.surfaced && target && chebyshev(w.x, w.y, target.x, target.y) <= 1) {
      return { kind: "surprise_breach", targetGuestId: target.guestId };
    }
    // Breach toward target (or random if no target).
    if (target) {
      const step = stepToward(w.x, w.y, target.x, target.y);
      if (step) return { kind: "breach", dx: step.dx, dy: step.dy, steps: config.whaleBreachRange };
    }
    return { kind: "breach", dx: 0, dy: 1, steps: 1 };
  }

  // We can submerge. Prefer Bubble Move occasionally (to scare whalers with
  // a hint zone), else Deep Dive for full stealth.
  // Use Bubble exactly once per submerged stretch (bubbledThisStretch flag
  // is reset on surface). 30% chance of bubble when allowed, 70% deep dive.
  const wantBubble = !w.bubbledThisStretch && Math.random() < 0.3;
  const tgt = target ?? randomWaterCell(s);
  const step = stepToward(w.x, w.y, tgt.x, tgt.y) ?? { dx: 0, dy: -1 };
  if (wantBubble) {
    return { kind: "bubble_move", dx: step.dx, dy: step.dy, steps: config.whaleBubbleRange };
  }
  return { kind: "deep_dive", dx: step.dx, dy: step.dy, steps: config.whaleDeepDiveRange };
}

// Whaler decision ----------------------------------------------------------

function botDecideShip(s: WhalabroadState, ship: ShipState): ShipPending {
  if (!s.whale) return { kind: "pass" };
  const w = s.whale;

  // Phase 2: whale is dead, race to harbor.
  if (w.dead) {
    const towingMe = w.towedBy === ship.guestId;
    if (towingMe) {
      // Crawl toward the nearest harbor cell.
      const [hx, hy] = s.harbor[0] ?? [6, 11];
      const step = stepToward(ship.x, ship.y, hx, hy);
      if (step) return { kind: "slow_crawl", dx: step.dx, dy: step.dy, steps: 1 };
      return { kind: "pass" };
    }
    // Try to harpoon if adjacent and corpse is unclaimed.
    if (!w.towedBy && chebyshev(ship.x, ship.y, w.x, w.y) <= config.shipHarpoonRange) {
      return { kind: "harpoon_corpse" };
    }
    // Otherwise approach the corpse. If a rival is towing, intercept.
    const step = stepToward(ship.x, ship.y, w.x, w.y);
    if (step) return { kind: "full_sail", dx: step.dx, dy: step.dy, steps: config.shipFullSailRange };
    return { kind: "pass" };
  }

  // Phase 1: whale alive. Heal if low HP and near harbor.
  const inRepairZone = s.harbor.some(([hx, hy]) =>
    Math.abs(hx - ship.x) + Math.abs(hy - ship.y) <= config.harborRepairRadius);
  if (ship.hp <= 1 && inRepairZone) return { kind: "repair" };

  // Whale visible (surfaced)?
  if (w.surfaced) {
    const dist = chebyshev(ship.x, ship.y, w.x, w.y);
    // Try to fire if within cannon range AND target is on a perpendicular line
    // relative to current facing. Compute which side: port is facing+6%8, starboard is facing+2%8.
    const dxAbs = Math.abs(w.x - ship.x);
    const dyAbs = Math.abs(w.y - ship.y);
    const inLine = (dxAbs === 0 && dyAbs <= config.shipCannonRange) ||
                   (dyAbs === 0 && dxAbs <= config.shipCannonRange) ||
                   (dxAbs === dyAbs && dxAbs <= config.shipCannonRange);
    if (dist <= config.shipCannonRange && inLine) {
      // Pick port or starboard based on which side the whale is on relative
      // to facing. Just try port; if invalid the server rejects and we'll
      // try a different angle next turn.
      const portFacing = (ship.facing + 6) % 8;
      const stbdFacing = (ship.facing + 2) % 8;
      const tgtFacing = directionToFacing(Math.sign(w.x - ship.x), Math.sign(w.y - ship.y));
      const side: "port" | "starboard" =
        tgtFacing === portFacing ? "port" :
        tgtFacing === stbdFacing ? "starboard" :
        Math.random() < 0.5 ? "port" : "starboard";
      return { kind: "fire_cannons", side, targetX: w.x, targetY: w.y };
    }
    // In range but not in line — slow-crawl to align facing.
    if (dist <= config.shipCannonRange + 1) {
      const step = stepToward(ship.x, ship.y, w.x, w.y);
      if (step) return { kind: "slow_crawl", dx: step.dx, dy: step.dy, steps: 1 };
    }
    // Out of range — full-sail toward.
    const step = stepToward(ship.x, ship.y, w.x, w.y);
    if (step && !ship.recoilUntilTurn) return { kind: "full_sail", dx: step.dx, dy: step.dy, steps: config.shipFullSailRange };
    if (step) return { kind: "slow_crawl", dx: step.dx, dy: step.dy, steps: 1 };
  }

  // Whale hidden — patrol toward last bubble or center of board.
  const center = Math.floor(BOARD_SIZE / 2);
  const step = stepToward(ship.x, ship.y, center, Math.floor(center / 2));
  if (step) return { kind: "slow_crawl", dx: step.dx, dy: step.dy, steps: 1 };
  return { kind: "pass" };
}
...
function botDecideGhost(s: WhalabroadState, ghost: ShipState): ShipPending {
  if (!s.whale) return { kind: "pass" };
  // Use kraken if available — target the densest cluster of LIVING ships
  // (we want PvP chaos in phase 2; pre-kill we'd rather hit the whale).
  if (!ghost.krakenUsed) {
    const target = !s.whale.dead && s.whale.surfaced ? s.whale :
                   aliveShips(s).filter(sh => sh.guestId !== ghost.guestId)[0];
    if (target) {
      return { kind: "kraken_summon", targetX: target.x, targetY: target.y };
    }
  }
  // Otherwise whisper a hint to the first living ally about whale position
  // (only useful before whale is dead).
  const ally = aliveShips(s)[0];
  if (ally && !s.whale.dead) {
    return { kind: "whisper", whisperTo: ally.guestId,
             whisperText: `Whale at ~(${s.whale.x}, ${s.whale.y})` };
  }
  return { kind: "pass" };
}

// Public entrypoint — called by resolveTurn for any bot that hasn't queued
// an action this turn.
export function botActionFor(s: WhalabroadState, guestId: string): WhalePending | ShipPending | null {
  if (!isBot(s, guestId)) return null;
  if (s.whale && s.whale.guestId === guestId) {
    return botDecideWhale(s);
  }
  const ship = s.ships.find(sh => sh.guestId === guestId);
  if (!ship) return null;
  if (ship.sunk) return botDecideGhost(s, ship);
  return botDecideShip(s, ship);
}
