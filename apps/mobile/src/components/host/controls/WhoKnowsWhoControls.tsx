...
export function WhoKnowsWhoControls({ viewMode, onViewModeChange }: Props) {
  ...
  const currentQ = expState?.currentQ;
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.phaseLabel}>WHO KNOWS WHO — {phase.toUpperCase()}</Text>
      ...
      {phase === "waiting" && (
        ...
        >
          <LinearGradient colors={["#ec4899", "#be185d"]} style={s.btnInner}>
      ...
      {phase === "question" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>QUESTION</Text>
          <Text style={s.questionText}>{currentQ?.text ?? "—"}</Text>
          <Text style={s.subText}>
            {voteCount} / {members.length} votes received
          </Text>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>ROUND COMPLETE</Text>
