...
export function TruthOrDareControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "spinning" && (
        <TouchableOpacity style={s.btn} onPress={() => sendAction("spin", { guestIds: guestIds() })}>
          <LinearGradient colors={["#b5179e", "#7209b7"]} style={s.btnI}><Text style={s.btnT}>🌀 SPIN</Text></LinearGradient>
        </TouchableOpacity>
      )}
      ...
      {phase === "playing" && (
        <View style={s.challengeBox}>
          <View style={s.badgeRow}>
            <View style={[s.badge, expState?.challengeType === "dare" ? s.badgeDare : s.badgeTruth]}>
              <Text style={s.badgeT}>{(expState?.challengeType ?? "truth").toUpperCase()}</Text>
            </View>
            <Text style={s.playerLabel}>{memberName(expState?.currentPlayer)}</Text>
          </View>
          <Text style={s.challengeText}>{expState?.currentChallenge ?? "—"}</Text>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  challengeBox: { backgroundColor: "#111", borderRadius: 12, padding: 14, gap: 8 },
  badgeRow: { flexDirection: "row", alignItems: "center", gap: 8 },
  badge: { borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  badgeTruth: { backgroundColor: "#1e40af" },
  badgeDare: { backgroundColor: "#9f1239" },
  badgeT: { color: "#fff", fontSize: 10, fontWeight: "900" },
  playerLabel: { color: "#a78bfa", fontSize: 13, fontWeight: "700" },
  challengeText: { color: "#fff", fontSize: 15, lineHeight: 22 },
});
