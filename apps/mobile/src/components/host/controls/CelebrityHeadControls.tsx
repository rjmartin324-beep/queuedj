...
export function CelebrityHeadControls({ viewMode, onViewModeChange }: Props) {
      ...
      {phase === "playing" && (
        <View style={s.card}>
          <View style={s.hostOnly}>
            <Text style={s.hostOnlyLabel}>🔒 HOST EYES ONLY</Text>
            <Text style={s.celebrity}>{expState?.celebrity ?? "—"}</Text>
          </View>
          <View style={s.row}>
            <Text style={s.name}>Current Player</Text>
            <Text style={s.pts}>{memberName(expState?.currentGuestId)}</Text>
          </View>
          <View style={s.row}>
            <Text style={s.name}>Questions Asked</Text>
            <Text style={s.pts}>{expState?.questionsAsked ?? 0}</Text>
          </View>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  hostOnly: { backgroundColor: "#1a0a2e", borderRadius: 8, padding: 12, borderWidth: 1, borderColor: "#7209b7", alignItems: "center", gap: 4 },
  hostOnlyLabel: { color: "#7209b7", fontSize: 9, fontWeight: "900", letterSpacing: 1.2 },
  celebrity: { color: "#f0abfc", fontSize: 22, fontWeight: "900", textAlign: "center" },
});
