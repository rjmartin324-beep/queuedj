...
export function MusicalChairsControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "music_playing" && (
        <View style={s.card}>
          <View style={s.row}>
            <Text style={s.name}>Active Players</Text>
            <Text style={s.pts}>{expState?.activePlayers?.length ?? 0}</Text>
          </View>
          <TouchableOpacity style={s.btn} onPress={() => sendAction("music_stop")}>
            <LinearGradient colors={["#b5179e", "#7209b7"]} style={s.btnI}><Text style={s.btnT}>🛑 STOP MUSIC</Text></LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "scramble" && (
        <View style={s.card}>
          <Text style={s.info}>
            Taps: {Object.keys(expState?.taps ?? {}).length}
          </Text>
          <TouchableOpacity style={s.smBtn} onPress={() => sendAction("end_scramble")}>
            <Text style={s.smBtnT}>⏹ END SCRAMBLE</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "elimination" && (
        <View style={s.card}>
          <Text style={s.label}>ELIMINATED</Text>
          <Text style={s.eliminatedText}>
            {memberName(expState?.eliminated?.[expState.eliminated.length - 1])}
          </Text>
          <TouchableOpacity style={s.smBtn} onPress={() => sendAction("next_round")}>
...
const s = StyleSheet.create({
  ...
  eliminatedText: { color: "#f87171", fontSize: 20, fontWeight: "900", textAlign: "center" },
});
