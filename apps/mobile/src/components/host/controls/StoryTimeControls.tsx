...
export function StoryTimeControls({ viewMode, onViewModeChange }: Props) {
  ...
  const storyWords: string[] = expState?.storyWords ?? [];
  const currentTurnId: string = expState?.currentTurn ?? "";
  const currentTurnName =
    members.find((m) => m.guestId === currentTurnId)?.displayName ??
    currentTurnId.slice(0, 6);
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      ...
      {phase === "playing" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>CURRENT TURN</Text>
          <Text style={s.questionText}>{currentTurnName}</Text>
          {storyWords.length > 0 && (
            <Text style={s.subText}>Story so far: {storyWords.join(" ")}</Text>
          )}
          <TouchableOpacity style={s.btn} onPress={() => sendAction("timeout")}>
            <LinearGradient colors={["#f59e0b", "#d97706"]} style={s.btnInner}>
              <Text style={s.btnText}>SKIP TURN</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "finished" && (
        <View style={s.card}>
          <Text style={s.finishedTitle}>STORY COMPLETE</Text>
          {storyWords.length > 0 && (
            <Text style={s.subText}>{storyWords.join(" ")}</Text>
          )}
        </View>
      )}
