...
export function AlibiControls({ viewMode, onViewModeChange }: Props) {
  ...
  const currentCase = expState?.currentCase;
  const voteCount: number = Object.keys(expState?.votes ?? {}).length;
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.phaseLabel}>ALIBI — {phase.toUpperCase()}</Text>
      ...
      {phase === "waiting" && (
        ...
        >
          <LinearGradient colors={["#475569", "#334155"]} style={s.btnInner}>
      ...
      {phase === "reading" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>THE CRIME</Text>
          <Text style={s.questionText}>{currentCase?.crime ?? "—"}</Text>
          <TouchableOpacity style={s.btn} onPress={() => sendAction("ready_to_vote")}>
            <LinearGradient colors={["#475569", "#334155"]} style={s.btnInner}>
              <Text style={s.btnText}>READY TO VOTE</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "voting" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>VOTING</Text>
          <Text style={s.subText}>
            {voteCount} / {members.length} votes cast
          </Text>
          <TouchableOpacity style={s.btn} onPress={() => sendAction("reveal")}>
            <LinearGradient colors={["#475569", "#334155"]} style={s.btnInner}>
              <Text style={s.btnText}>REVEAL CULPRIT</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>CULPRIT REVEALED</Text>
          <TouchableOpacity style={s.btn} onPress={() => sendAction("next")}>
            <LinearGradient colors={["#475569", "#334155"]} style={s.btnInner}>
              <Text style={s.btnText}>NEXT CASE ▶</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "finished" && (
        <View style={s.card}>
          <Text style={s.finishedTitle}>CASE CLOSED</Text>
