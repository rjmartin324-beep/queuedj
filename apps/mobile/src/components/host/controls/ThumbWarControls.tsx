...
export function ThumbWarControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "playing" && (
        <View style={s.card}>
          <Text style={s.label}>LIVE TAP COUNTS</Text>
          {Object.entries(expState?.tapCounts ?? {})
            .sort(([, a], [, b]) => (b as number) - (a as number))
            .map(([gId, count]) => (
              <View key={gId} style={s.tapRow}>
                <Text style={s.name}>{memberName(gId)}</Text>
                <Text style={s.tapCount}>{count as number} 👍</Text>
              </View>
            ))}
          <TouchableOpacity style={s.btn} onPress={() => sendAction("end_round")}>
            <LinearGradient colors={["#b5179e", "#7209b7"]} style={s.btnI}><Text style={s.btnT}>🛑 END ROUND</Text></LinearGradient>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "round_end" && (
        <View style={s.card}>
          <Text style={s.label}>ROUND WINNER</Text>
          <Text style={s.winnerText}>{memberName(expState?.roundWinner)}</Text>
...
const s = StyleSheet.create({
  ...
  tapRow: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 4 },
  tapCount: { color: "#fbbf24", fontSize: 14, fontWeight: "900" },
  winnerText: { color: "#4ade80", fontSize: 22, fontWeight: "900", textAlign: "center" },
});
