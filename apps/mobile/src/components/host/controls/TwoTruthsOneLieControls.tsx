...
export function TwoTruthsOneLieControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "submitting" && (
        <View style={s.card}>
          <Text style={s.info}>
            Waiting for {memberName(expState?.currentSubmitter)} to submit facts
          </Text>
        </View>
      )}
      ...
      {phase === "voting" && (
        <View style={s.card}>
          <Text style={s.info}>
            Votes: {Object.keys(expState?.votes ?? {}).length}/{members.length}
          </Text>
          <TouchableOpacity style={s.smBtn} onPress={() => sendAction("reveal_lie", { lieIndex: expState?.lieIndex ?? 2 })}>
            <Text style={s.smBtnT}>👁 REVEAL LIE</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          ...
          {expState?.results && Object.entries(expState.results).map(([gId, correct]) => (
            <View key={gId} style={s.row}>
              <Text style={s.name}>{memberName(gId)}</Text>
              <Text style={{ color: correct ? "#4ade80" : "#f87171", fontSize: 13, fontWeight: "700" }}>
