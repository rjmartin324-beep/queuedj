...
export function LyricsDropControls({ viewMode, onViewModeChange }: Props) {
  ...
  const lyricSnippet: string = expState?.lyricSnippet ?? "—";
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.phaseLabel}>LYRICS DROP — {phase.toUpperCase()}</Text>
      ...
      {phase === "waiting" && (
        ...
        >
          <LinearGradient colors={["#22c55e", "#16a34a"]} style={s.btnInner}>
      ...
      {phase === "question" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>LYRIC SNIPPET</Text>
          <Text style={s.questionText}>"{lyricSnippet}"</Text>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>CORRECT ANSWER</Text>
          <Text style={s.questionText}>{correctAnswer}</Text>
