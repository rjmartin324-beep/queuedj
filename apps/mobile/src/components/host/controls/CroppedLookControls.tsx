...
export function CroppedLookControls({ viewMode, onViewModeChange }: Props) {
  ...
  const currentPuzzle = expState?.currentPuzzle;
  const correctGuessers: string[] = expState?.correctGuessers ?? [];
  const emojiDisplay: string = Array.isArray(currentPuzzle?.emojis)
    ? currentPuzzle.emojis.join("")
    : "—";
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.phaseLabel}>CROPPED LOOK — {phase.toUpperCase()}</Text>
      ...
      {phase === "waiting" && (
        ...
        >
          <LinearGradient colors={["#0d9488", "#0f766e"]} style={s.btnInner}>
      ...
      {phase === "question" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>PUZZLE</Text>
          <Text style={s.emojiDisplay}>{emojiDisplay}</Text>
          <Text style={s.subText}>
            {correctGuessers.length} correct so far
          </Text>
          <View style={s.btnRow}>
            <TouchableOpacity style={[s.btn, s.flex1]} onPress={() => sendAction("more_reveal")}>
              <LinearGradient colors={["#0d9488", "#0f766e"]} style={s.btnInner}>
                <Text style={s.btnText}>ADD HINT</Text>
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity style={[s.btn, s.flex1]} onPress={() => sendAction("reveal")}>
              <LinearGradient colors={["#0d9488", "#0f766e"]} style={s.btnInner}>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          ...
          <Text style={s.questionText}>{currentPuzzle?.answer ?? "—"}</Text>
          <TouchableOpacity style={s.btn} onPress={() => sendAction("next")}>
            <LinearGradient colors={["#0d9488", "#0f766e"]} style={s.btnInner}>
              <Text style={s.btnText}>NEXT PUZZLE ▶</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  emojiDisplay: { fontSize: 32, textAlign: "center", letterSpacing: 4 },
