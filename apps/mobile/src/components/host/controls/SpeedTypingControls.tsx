...
export function SpeedTypingControls({ viewMode, onViewModeChange }: Props) {
  ...
  const finishCount: number = Object.values(expState?.finishers ?? {}).filter(Boolean).length;
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.phaseLabel}>SPEED TYPING — {phase.toUpperCase()}</Text>
      ...
      {phase === "waiting" && (
        ...
        >
          <LinearGradient colors={["#3b82f6", "#2563eb"]} style={s.btnInner}>
      ...
      {phase === "typing" && (
        <View style={s.card}>
          <Text style={s.roundLabel}>CURRENT PHRASE</Text>
          <Text style={s.questionText}>{currentPhrase}</Text>
          <Text style={s.subText}>
            {finishCount} / {members.length} finished
          </Text>
