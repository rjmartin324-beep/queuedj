...
export function SpeedRoundControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "playing" && (
        <View style={s.card}>
          {expState?.currentChallenge?.category && (
            <View style={s.badge}>
              <Text style={s.badgeT}>{expState.currentChallenge.category.toUpperCase()}</Text>
            </View>
          )}
          <Text style={s.challengeText}>{expState?.currentChallenge?.challenge ?? "—"}</Text>
          <View style={s.completionRow}>
            <Text style={s.completionLabel}>COMPLETED</Text>
            <Text style={s.completionCount}>
              {Object.values(expState?.completions ?? {}).filter(Boolean).length}/{members.length}
            </Text>
          </View>
...
const s = StyleSheet.create({
  ...
  challengeText: { color: "#fff", fontSize: 16, lineHeight: 24, fontWeight: "700" },
  completionRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  completionLabel: { color: "#444", fontSize: 9, fontWeight: "900", letterSpacing: 1.2 },
  completionCount: { color: "#4ade80", fontSize: 16, fontWeight: "900" },
});
