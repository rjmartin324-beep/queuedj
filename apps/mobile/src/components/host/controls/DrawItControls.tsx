  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "drawing" && (
        <View style={s.card}>
          <View style={s.row}>
            <Text style={s.name}>Drawer</Text>
            <Text style={s.pts}>{memberName(expState?.currentDrawer)}</Text>
          </View>
          <View style={s.hostOnly}>
            <Text style={s.hostOnlyLabel}>🔒 PROMPT (HOST ONLY)</Text>
            <Text style={s.promptText}>{expState?.prompt ?? "—"}</Text>
          </View>
          <TouchableOpacity style={s.smBtn} onPress={() => sendAction("drawing_done")}>
            <Text style={s.smBtnT}>⏭ SKIP</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {phase === "guessing" && (
        <View style={s.card}>
          <Text style={s.info}>
            Guesses: {Object.keys(expState?.guesses ?? {}).length}/{members.length - 1}
          </Text>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          ...
          <Text style={s.answerText}>{expState?.correctAnswer ?? "—"}</Text>
...
const s = StyleSheet.create({
  ...
  hostOnly: { backgroundColor: "#1a0a2e", borderRadius: 8, padding: 10, borderWidth: 1, borderColor: "#7209b7", alignItems: "center", gap: 4 },
  hostOnlyLabel: { color: "#7209b7", fontSize: 9, fontWeight: "900", letterSpacing: 1.2 },
  promptText: { color: "#f0abfc", fontSize: 18, fontWeight: "900", textAlign: "center" },
  answerText: { color: "#4ade80", fontSize: 20, fontWeight: "900", textAlign: "center" },
});
