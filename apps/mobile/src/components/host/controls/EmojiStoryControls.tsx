...
export function EmojiStoryControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "question" && (
        <View style={s.card}>
          <Text style={s.emojiDisplay}>{expState?.currentPuzzle?.emojis ?? "—"}</Text>
          <Text style={s.info}>
            Guesses: {Object.keys(expState?.guesses ?? {}).length}/{members.length}
          </Text>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          ...
          <Text style={s.answerText}>{expState?.currentPuzzle?.answer ?? "—"}</Text>
