...
export function RankItControls({ viewMode, onViewModeChange }: Props) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.content}>
      ...
      {phase === "ranking" && (
        <View style={s.card}>
          <Text style={s.label}>ITEMS TO RANK</Text>
          <Text style={s.itemsText}>{expState?.currentChallenge?.items?.join(", ") ?? "—"}</Text>
          <Text style={s.info}>
            Submissions: {Object.keys(expState?.submissions ?? {}).length}/{members.length}
          </Text>
      ...
      {phase === "reveal" && (
        <View style={s.card}>
          <Text style={s.label}>CORRECT ORDER</Text>
          {(expState?.currentChallenge?.correctOrder ?? expState?.currentChallenge?.items ?? []).map((item: string, i: number) => (
            <View key={i} style={s.orderRow}>
              <Text style={s.orderNum}>#{i + 1}</Text>
              <Text style={s.orderItem}>{item}</Text>
            </View>
          ))}
...
const s = StyleSheet.create({
  ...
  itemsText: { color: "#ccc", fontSize: 13, lineHeight: 20 },
  orderRow: { flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 2 },
  orderNum: { color: "#a78bfa", fontSize: 12, fontWeight: "900", width: 24 },
  orderItem: { color: "#ccc", fontSize: 13 },
});
